...
|---|---|---|
| **Активных пользователей в день** | 50 млн | Типовое глобальное мобильное приложение |
| **Сигналов в день** | 500 млн (10 на пользователя) | SDK буферирует, часть совмещается в batch |
| **RPS пик** | 100 000 | 500M / 86400 s = 5.8K base, пик ~17x |
| **Размер события** | 2 КБ avg | Логи 500Б, крашдампы 10КБ, батарея 100Б |
| **Пропускная способность** | 200 ГБ/день | 1 ТБ raw → 200 ГБ compressed |
...
|---|---|---|
| **RPS базовый** | 500M активных × 10 событий / 86400 s | 5 800 RPS |
| **RPS пик** | 5800 × 17x | 100 000 RPS |
| **Пропускная способность (raw)** | 500M × 2 КБ / 86400 s | 12 ГБ/сек |
| **Пропускная способность (сжатая)** | 12 ГБ × 0.2 | 2.4 ГБ/сек (~200 ГБ/день) |
